...
const Operator * OperatorTable::find(std::string_view op_name) const
{
	std::string s_op_name(op_name);
...
void OperatorTable::register_operator(std::string name, std::unique_ptr<Operator> op)
{
	operators.insert({ name, std::move(op) });
}
void OperatorTable::register_un_implementation(std::string name, std::unique_ptr<OperatorFunction> op_func)
{
	unary_implemetations.insert({ name, std::move(op_func) });
}
void OperatorTable::register_bin_implementation(std::string name, std::unique_ptr<OperatorFunction> op_func)
{
...
void OperatorTable::register_builtin_operators(void)
{
	#define REG_OP(name, fixity, precedence) \
		register_operator(name, std::make_unique<Operator>(name, fixity, precedence))

	#define REG_UN_IMP(name, implementation, type1, typ2, ret_type) \
		register_un_implementation(name, std::make_unique<OperatorFunction>(OperatorType::O_BUILTIN, implementation, std::make_pair<MathObjType, MathObjType>(type1, typ2), ret_type))
	#define REG_BIN_IMP(name, implementation, type1, typ2, ret_type) \
		register_bin_implementation(name, std::make_unique<OperatorFunction>(OperatorType::O_BUILTIN, implementation, std::make_pair<MathObjType, MathObjType>(type1, typ2), ret_type))
